...
## SYSTEM STATUS (as of 2026-04-30)
  ...
  ### 2026-05-02 — PWA Phase A Step 3b-1 deployed (frontend redemption)

  Frontend portal pages now redeem token → session_id on
  first mount and use session_id for subsequent calls.
  Token-only fallback preserved if redemption fails.

  Files modified:
    - src/lib/portal-session.ts (NEW — helper utility:
      getPortalSession, setPortalSession, clearPortalSession,
      redeemPortalToken)
    - src/pages/CustomerPortal.tsx — session state hydrated
      from localStorage on mount; useEffect redeems URL token
      if no cached session; fetchPortal updated to use
      session_id with token fallback + 401 self-heal;
      verify-portal-pin / submit-payment /
      edit-payment-submission / customer-portal POST all
      send session_id when present
    - src/pages/LoyaltyPortal.tsx — same pattern via
      useQuery; fetchPortal accepts auth object; queryKey
      uses token (stable) so existing
      invalidateQueries({queryKey:['portal',portalToken]})
      sites continue to work
    - src/components/portal/CashPortalPaymentDialog.tsx —
      reads getPortalSession() at submit time; falls back to
      portal_token prop
    - src/components/loyalty/LoyaltyJoinPrompt.tsx — same
      pattern

  localStorage key: cha_portal_session (JSON-encoded
  PortalSession with session_id, customer_id, expires_at,
  redeemed_at).

  Pattern decision: child components do NOT receive session
  via prop; they read getPortalSession() from localStorage at
  fetch time. This avoids prop-plumbing through 5+ levels of
  nesting (AccountDetail and its children) and matches the
  "session is the device's auth, not the page's" mental
  model. Trade-off: cannot test session-aware fetch in
  isolation without setting localStorage. Acceptable.

  Token rotation handling: when session is set but the
  customer-portal GET returns 401, fetchPortal clears
  localStorage, drops session state, and retries with the
  URL token (still in the URL). If URL token also fails,
  customer sees "Access denied" error.

  Backwards compatibility: customers without localStorage
  support (private browsing) silently fall back to token
  auth. Customers visiting old Messenger links continue to
  work — first visit redeems, subsequent visits use cached
  session.

  extension_requests INSERT at CustomerPortal.tsx:1402
  intentionally untouched. Its portal_token column stores
  the token for forensic traceability of which link was used
  to file the extension request — that's a row data field,
  not auth.

  Phase A status:
    - Step 1 (table): COMPLETE
    - Step 2 (helper + redeem): COMPLETE
    - Step 3a-1 through 3a-3b (backend): COMPLETE
    - Step 3b-1 (frontend redemption): THIS COMMIT
    - Step 3b-2 (/launch + InstallAppBanner): PENDING
    - Step 3b-3 (manifest start_url): PENDING

  Frontend-only commit. Auto-deploys via Firebase Hosting.
  Single revert restores token-only frontend behavior;
  backend remains backwards-compatible regardless.

  ### 2026-05-03 — PWA Phase A Step 3b-2 deployed (/launch + banner)

  New /launch route and InstallAppBanner component.

  Files created:
    - src/pages/Launch.tsx — 3-case redirect logic with
      AuthContext loading-state guard
    - src/components/portal/InstallAppBanner.tsx —
      beforeinstallprompt listener (Chrome/Edge/Android) +
      iOS Safari instructional fallback + dismissible via
      cha_install_banner_dismissed localStorage flag

  Files modified:
    - src/App.tsx — lazyWithRetry import + public Route
      between /unsubscribe and Protected '/'
    - src/pages/CustomerPortal.tsx — InstallAppBanner mount
      inside main content container with TEST-% gate
      computed from data.accounts
    - src/pages/LoyaltyPortal.tsx — PortalData interface
      extended with optional accounts field; InstallAppBanner
      mount inside MemberView render branch only (JoinPrompt
      and ComingSoon branches skipped — not natural install
      contexts)

  /launch logic:
    Case (a) — getPortalSession() returns valid → /portal
    Case (b) — useAuth() user has roles AND not loading → /
    Case (c) — fallback message (open from Messenger link)

  /launch is currently UNREACHABLE from normal navigation.
  Only the manifest start_url change in Step 3b-3 will make
  it reachable from PWA installs. Direct visits to /launch
  before Step 3b-3 land on case (c) fallback.

  AuthContext loading guard: case (b) is gated on
  !loading so that admins arriving at /launch during the
  initial session-hydration window are not flashed with the
  case (c) fallback message before the redirect fires.

  InstallAppBanner gating:
  - Parent passes show prop based on accounts.invoice_number
    starting with 'TEST-'
  - Banner ALSO requires either deferredPrompt (Chrome) or
    iOS Safari user-agent — won't render if neither
  - Once dismissed, persists across reloads via localStorage
  - Production customers (no TEST-% accounts) NEVER see the
    banner regardless of browser

  Cleanup tracking: this work is the Phase A counterpart to
  the PR-1/PR-2 attempts that were reverted in Bug #65.
  PR-1's hide-on-customer-routes hotfix and PR-2's data:-URL
  dynamic manifest are both superseded by this implementation:
    - The banner now lives on portal pages directly (route
      gating is implicit because component is mounted inside
      portal page bodies)
    - The TEST-% gate keeps it invisible to production
      customers
    - Static manifest (no dynamic data: URLs) means Chrome's
      install eligibility heuristic actually fires; once Step
      3b-3 ships the start_url change, deferredPrompt will
      arrive and customers can install.

  Phase A status:
    - Step 1 (table): COMPLETE
    - Step 2 (helper + redeem): COMPLETE
    - Step 3a-1 through 3a-3b (backend): COMPLETE
    - Step 3b-1 (frontend redemption): COMPLETE
    - Step 3b-2 (/launch + InstallAppBanner): THIS COMMIT
    - Step 3b-3 (manifest start_url): PENDING

  Step 3b-3 will flip vite.config.ts manifest start_url
  to /launch in a follow-up commit. Manifest changes are
  sticky on installed PWAs, so 3b-3 ships only after this
  commit's /launch is verified working in production.
