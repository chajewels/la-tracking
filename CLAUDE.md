...
## PLAN LENGTH SOURCE — DERIVE FROM SCHEDULE (added 2026-05-20)

  planMonths is ALWAYS `MAX(installment_number)` over the account's
  NON-cancelled `layaway_schedule` rows. Never `payment_plan_months || 6`.
  Never `count(*)` — a deleted middle row must not change the final-month
  identity.

### Why
  `layaway_accounts.payment_plan_months` drifted historically:
    - `delete-installment` mutated the schedule without syncing the column.
    - Some accounts were created with a mismatched column from day one.
  When the column was wrong, the final-month penalty cap and auto-forfeit
  detection targeted the wrong installment_number:
    - 5-row plan with payment_plan_months=6 → its true final row (#5) got
      the ₱1,000/¥2,000 non-final cap instead of ₱3,000/¥6,000, under-
      capping the row that should be uncapped.
    - 4-row plan with payment_plan_months=3 → row #3 was treated as
      final and auto-forfeit triggered one row early when it crossed
      ₱3,000/¥6,000.
  8 accounts drifted historically (INV #18748 confirmed via a logged
  delete; the rest unlogged) — all backfilled 2026-05-20.

### Derivation rule
  ```ts
  planMonths = MAX(installment_number) over rows WHERE status <> 'cancelled'
  ```
  Frontend helper: `derivePlanMonths(scheduleRows)` in
  `src/lib/business-rules.ts`. `getPenaltyCap` / `isPenaltyOverCap` no
  longer carry a `= 6` default — callers MUST pass the derived value.

### Cache rule
  `layaway_accounts.payment_plan_months` is now a synced display cache
  only. It is written ONLY by:
    - `create-layaway-account` (initial set at account creation)
    - `add-installment` (set to `nextNumber` after a successful insert)
    - `delete-installment` (recomputed from remaining non-cancelled rows
      after a successful delete)
  Any other code path that needs the plan length MUST derive from the
  schedule, never read the column. UI components MAY fall back to the
  column when the schedule isn't loaded yet, but should prefer the
  derived value wherever rows are available.

### Sites converted to schedule-derived (2026-05-20)
  - `penalty-engine/index.ts` — per-account planMonths Map fetched
    alongside the freeze guard; `installmentNumber === planMonths`
    aligned with `getPenaltyCap` line 63.
  - `add-penalty/index.ts` — derived from a one-shot non-cancelled
    schedule query per call.
  - `finance-reconciliation/index.ts` — derived from the already-loaded
    `scheds` array (already non-cancelled per the upstream fetch).
  - `auto-forfeit-settlement/index.ts` final-month detection — replaced
    `schedItems.find(... === payment_plan_months) || schedItems[last]`
    with simply `schedItems[schedItems.length - 1]`. schedItems is
    already non-cancelled + sorted ascending, so its last element IS
    the true final row.
    (Extension-month detection at line 233 still reads
    `payment_plan_months + 1`; it now relies on the cache being synced
    by add-installment / delete-installment.)
  - `src/lib/business-rules.ts` — `getPenaltyCap` / `isPenaltyOverCap`
    `= 6` default removed; new `derivePlanMonths` helper exported.
  - `src/pages/AccountDetail.tsx` — single `planMonths` const derived
    from `scheduleItems` at the top of the component; threaded through
    `ApplyPenaltyCapDialog`, `isPenaltyOverCap`, header label, and
    invoice generator default. EditAccountDialog's form payload still
    reads the column (out of scope; the dialog is its own consumer).
  - `src/components/dashboard/PenaltyCapAuditPanel.tsx` — derived from
    embedded `layaway_schedule`.

### Forbidden
  - Reading `account.payment_plan_months` in penalty/forfeit logic.
  - Defaulting planMonths to 6 (or any constant) when the schedule is
    available.
  - Counting `scheduleRows.length` — middle-row deletes break that.
