// set-account-deadlines — staff move a deposit deadline or a settlement date.
//
...
//
// One control, both tables: `layaway` moves transfer_due_at and
// settlement_due_at on layaway_accounts, `cash_order` moves transfer_due_at AND
// expires_at on cash_orders, because the expiry cron reads expires_at and moving
// only one would show the customer a new deadline while the cron still cancelled
// on the old one.
//
// Everything is done inside set_account_deadlines: the live-status check, both
// writes and the audit row with the old and new values and the reason.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const settlementDueAt = body?.settlement_due_at ?? null;
    if (settlementDueAt !== null && Number.isNaN(Date.parse(String(settlementDueAt)))) {
      return jsonResponse({ error: "settlement_due_at is not a valid timestamp" }, 400);
    }
