...
const PaymentSubmittedEmail = ({
  ...
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Payment submitted for INV #{invoiceNumber}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={headerBar}>
            <Text style={brandText}>💎 {SITE_NAME}</Text>
          </Section>
          <Heading style={{ ...h1, color: '#2563eb' }}>Payment Submitted Successfully</Heading>
          <Text style={greeting}>Hi {customerName},</Text>
          <Text style={text}>
            Your payment of <strong>{symbol} {amountPaid}</strong> for{' '}
            <strong>INV #{invoiceNumber}</strong> has been submitted and is
          ...
          {portalUrl && (
            <Section style={{ textAlign: 'center' as const, margin: '24px 0' }}>
              <Button style={{ ...button, backgroundColor: '#2563eb' }} href={portalUrl}>
                View My Account
              </Button>
            </Section>
          )}
