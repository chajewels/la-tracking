...
const PaymentReceiptEmail = ({
  ...
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Payment recorded for INV #{invoiceNumber}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={headerBar}>
            <Text style={brandText}>💎 {SITE_NAME}</Text>
          </Section>
          <Heading style={{ ...h1, color: '#16a34a' }}>Payment Recorded</Heading>
          <Text style={greeting}>Hi {customerName},</Text>
          <Text style={text}>
            Your payment of <strong>{symbol} {amountPaid}</strong> for{' '}
            <strong>INV #{invoiceNumber}</strong> has been recorded by our team.
            Thank you for your continued trust in {SITE_NAME}! 🧡
          </Text>
...
export const template = {
  ...
  subject: (data: Record<string, any>) =>
    `✅ Payment Recorded — INV #${data.invoiceNumber || ''}`,
  displayName: 'Payment receipt',
  previewData: {
    customerName: 'Maria Santos',
    invoiceNumber: '18456',
    amountPaid: '4,500',
    paymentDate: 'April 23, 2026',
    paymentMethod: 'GCash',
    currency: 'PHP',
    remainingBalance: '9,000',
  },
