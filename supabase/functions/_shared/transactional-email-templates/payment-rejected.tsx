...
const PaymentRejectedEmail = ({
  ...
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Payment rejected for INV #{invoiceNumber}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={headerBar}>
            <Text style={brandText}>💎 {SITE_NAME}</Text>
          </Section>
          <Heading style={{ ...h1, color: '#dc2626' }}>Payment Rejected</Heading>
          <Text style={greeting}>Hi {customerName},</Text>
          <Text style={text}>
            Unfortunately your payment of <strong>{symbol} {amountPaid}</strong>{' '}
            for <strong>INV #{invoiceNumber}</strong> has been rejected.
          </Text>
          {rejectionReason && (
          ...
          <Text style={text}>
            Please resubmit your payment with the correct proof of payment via
            the customer portal.
          </Text>
          {portalUrl && (
...
export const template = {
  ...
  subject: (data: Record<string, any>) =>
    `❌ Payment Rejected — INV #${data.invoiceNumber || ''}`,
  ...
  previewData: {
    ...
    amountPaid: '4,500',
    currency: 'PHP',
    rejectionReason: 'Proof of payment is blurred and unreadable. Please resubmit a clear screenshot.',
  },
