...
--    WHERE table_name='cash_order_items' AND column_name='website_product_id';   -- 1 row
--   SELECT prosrc LIKE '%website_product_id%' FROM pg_proc
--    WHERE proname='create_web_order_atomic';                                    -- true
--   Then place one order from the storefront: cash_orders gains a source_channel='web'
--   row and cash_order_items a row with website_product_id set and product_id NULL.
