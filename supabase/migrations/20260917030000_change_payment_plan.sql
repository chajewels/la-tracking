-- Change payment plan (2026-09-17). Applied live via SQL Editor. change_payment_plan_atomic + change_payment_plan permission rows.
...
DECLARE
  ...
  v_n int; v_new_id uuid; v_csr_lost int;
...
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM public.payment_submissions ps WHERE ps.account_id = p_account_id
             AND ps.status::text IN ('submitted', 'under_review', 'needs_clarification')) THEN
    RETURN jsonb_build_object('error', 'A payment submission for this account is waiting for review. Confirm or reject it first.');
  END IF;
  ...
  IF NOT p_apply THEN
    RETURN jsonb_build_object('preview', true, 'current_months', v_acc.payment_plan_months, 'new_months', p_new_months,
      ...
      'csr_notifications_removed', v_csr_lost, 'new_end_date', v_end, 'currency', v_acc.currency);
  END IF;
  ...
  VALUES ('layaway_account', p_account_id, 'change_payment_plan',
          ...
          jsonb_build_object('payment_plan_months', p_new_months, 'end_date', v_end, 'rows', v_new_rows,
                             'removed_rows', v_removed, 'reason', p_reason),
          p_user_id);

  RETURN jsonb_build_object('success', true, 'current_months', v_acc.payment_plan_months, 'new_months', p_new_months,
    'kept_installments', v_k, 'new_rows', v_new_rows, 'removed_rows', v_removed, 'new_end_date', v_end);
END;
$function$;
...
INSERT INTO public.role_permissions (role, permission_key, is_allowed) VALUES
  ('admin'::app_role,      'change_payment_plan', true),
  ('staff'::app_role,      'change_payment_plan', false),
  ('finance'::app_role,    'change_payment_plan', false),
  ('csr'::app_role,        'change_payment_plan', false),
  ('live_agent'::app_role, 'change_payment_plan', false)
...
SELECT p.user_id, 'change_payment_plan', true FROM public.profiles p
