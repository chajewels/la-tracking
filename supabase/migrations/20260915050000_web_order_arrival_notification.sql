...
-- Verification after applying — the labels, without writing anything:
--   SELECT public.notify_money_label(29754, 'PHP'),      -- ₱29,754
--          public.notify_money_label(679980, 'JPY'),     -- ¥679,980
--          public.notify_deadline_label(now() + interval '72 hours'),
--          public.notify_deadline_label(NULL);           -- no deposit deadline set
--
-- And the two web branches are present:
--   SELECT proname,
--          pg_get_functiondef(oid) ILIKE '%Website layaway placed%' AS layaway_web,
--          pg_get_functiondef(oid) ILIKE '%paid in full%'           AS cash_web
--     FROM pg_proc WHERE proname IN ('notify_account_created','notify_cash_order_created');
