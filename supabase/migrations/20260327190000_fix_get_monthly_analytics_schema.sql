-- Fix get_monthly_analytics: correct table/column names based on actual schema
-- payments table (not layaway_payments), date_paid (not payment_date),
-- voided_at IS NULL (not status != 'voided'), no payment_type column,
-- penalty payments tracked via penalty_fees table, no closed_at column.
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  collected AS (
    -- All non-voided payments excluding those that are penalty-only
    -- (penalty payments are counted separately via penalty_fees)
    SELECT
  ...
  forfeited AS (
    -- Forfeited accounts: use updated_at as the forfeiture month
    SELECT
  ...
  penalties AS (
    -- Penalty fees that have been paid
    SELECT
