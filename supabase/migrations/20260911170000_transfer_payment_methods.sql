...
-- ============================================================ VERIFICATION
--   -- table, constraints and policies exist
--   SELECT conname, pg_get_constraintdef(oid) FROM pg_constraint
--    WHERE conrelid = 'public.transfer_payment_methods'::regclass ORDER BY conname;
--   SELECT polname, cmd FROM pg_policies WHERE tablename = 'transfer_payment_methods';
--
--   -- what got copied (expect 0 rows if no structured details were ever saved)
--   SELECT region, method_type, label_en, sort_order, is_active
--     FROM public.transfer_payment_methods ORDER BY region, sort_order, created_at;
--
--   -- no invented details anywhere
--   SELECT count(*) FROM public.transfer_payment_methods
--    WHERE account_number IS NOT NULL OR wallet_number IS NOT NULL;
