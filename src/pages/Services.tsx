...
export default function Services() {
  ...
  // Tab state synced with ?tab=… per CLAUDE.md SIDEBAR ARCHITECTURE pattern.
  const initialTab = (searchParams.get('tab') as TabKey) ?? DEFAULT_TAB;
  const [tab, setTabState] = useState<TabKey>(
    VALID_TABS.includes(initialTab) ? initialTab : DEFAULT_TAB,
  );
  useEffect(() => {
    const t = (searchParams.get('tab') as TabKey) ?? DEFAULT_TAB;
    if (VALID_TABS.includes(t) && t !== tab) setTabState(t);
  }, [searchParams, tab]);
...
function ServiceJobsTab() {
  ...
  const { data: jobs = [], isLoading } = useQuery<ServiceJobRow[]>({
    ...
    queryFn: async () => {
      ...
      // applied across all financial dashboards.
