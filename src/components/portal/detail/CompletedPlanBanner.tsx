...
export default function CompletedPlanBanner({ currency, totalPaid, totalObligation }: CompletedPlanBannerProps) {
  return (
    <div className="rounded-xl bg-card shadow-[0_2px_12px_rgba(43,39,35,0.06)] p-6 sm:p-7 text-center">
      ...
      <p className="font-display text-2xl text-foreground">{pt('detail.completedTitle')}</p>
      <p className="text-sm text-muted-foreground mt-1">{pt('detail.completedThanks')}</p>
      <div className="flex items-center justify-center gap-8 mt-5">
        <div>
          <p className="text-[10px] uppercase text-muted-foreground" style={{ letterSpacing: '0.15em' }}>{pt('detail.completedTotalPaid')}</p>
        ...
        <div>
          <p className="text-[10px] uppercase text-muted-foreground" style={{ letterSpacing: '0.15em' }}>{pt('detail.completedTotalObligation')}</p>
