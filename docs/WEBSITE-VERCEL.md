...
| `POST /auth/customer` | **Customer JWT + API key** | Links or creates the `customers` row for the signed-in user, in this order: (1) already linked → 200; (2) a customer holds the verified email → link that unlinked row → 200, or 409 `email_already_linked` when another auth user owns it — no profile needed; (3) otherwise a customer is CREATED only from a profile. Body: `full_name` and `location` REQUIRED, `facebook_name`, `messenger_link`, `mobile_number` optional (trimmed, empty → null; `location` stored like the Hub: `Japan`, `Philippines`, or the country name). **422 `{ error: "profile_required" }`** when `full_name` or `location` is empty — nothing created, nobody notified; the storefront sends her to `/account/complete-profile`, which posts again with the profile. There is NO email-prefix name any more (removed 2026-09-24): a customer is always named by what she typed. 409 `{ error: "already_registered", message }` when the new customer would duplicate an existing one on full name, Facebook name, mobile or email (docs/SCHEMA-FACTS.md "Duplicate-customer prevention") — nothing created, staff notified; 500 `duplicate_check_failed` if the check itself fails. Does NOT auto-enrol in loyalty. DEPLOY ORDER: the 422 must never reach a storefront that cannot handle it — it shipped after cha-jewels-web #135 was live. |
