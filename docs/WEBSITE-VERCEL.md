...
| `POST /auth/customer` | **Customer JWT + API key** | Links or creates the `customers` row for the signed-in user. Body: `full_name`, plus OPTIONAL `facebook_name`, `messenger_link`, `mobile_number`, `location` (trimmed, empty → null; `location` stored like the Hub: `Japan`, `Philippines`, or the country name). 409 `email_already_linked` when another auth user owns that email. 409 `{ error: "already_registered", message }` when a NEW customer would duplicate an existing one on full name, Facebook name, mobile or email (docs/SCHEMA-FACTS.md "Duplicate-customer prevention") — nothing is created and staff are notified; 500 `duplicate_check_failed` if the check itself fails. Does NOT auto-enrol in loyalty. |
