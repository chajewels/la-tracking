...
Status: steps 1–2 BUILT, step 3 DEFERRED (owner decision 2026-09-13), step 4 BUILT and awaiting review (2026-09-14), step 7 added 2026-09-13, step 7 BUILT 2026-09-19 (Page365 half: backend + review UI).
...
7. **BUILT 2026-09-19 — the Page365 half is complete end to end** (backend: migration `20260919100000`, `page365-fetch-order`, extras folded into the two create functions; UI: Sales → "From Page365" → `/page365/review/:draftId`, `20260919140000_page365_draft_consume.sql`). The storefront-product half of this step is still NOT built. **Formerly BACKEND BUILT 2026-09-19** (migration `20260919100000_page365_import.sql`, edge function `page365-fetch-order`, extras folded into `create-layaway-account` / `create-cash-order`). The Page365 half of this step now exists server-side: link in, parsed draft out, creation through the existing functions. The confirmation UI and the storefront-product half are NOT built. **CSR parking area and Page365 / storefront fetch-and-attach.** Two entry points, one confirmation surface: paste a public Page365 invoice link → fetch → draft a Hub order; paste or pick a storefront product → fetch → draft a `website_products` record. Both land in a parking area where a CSR checks the parsed fields, edits anything, and confirms. NOTHING writes to a live order or a live product without that confirmation. Findings, verified 2026-09-13 by fetching both endpoints:
   ...
   - Invoice fields: `no` (= the Hub `invoice_number`; example 19668 against a highest Hub invoice of 19667, so the numbers are the same series and this is the join key), `stage`, `customer{name, phone, address, structural_address}`, `created_at`, `expires_on`, `price_subtotal` / `price_shipping` / `price_discount` / `campaign_discount` / `price_total` (the total is NET of both discounts; the lines and `price_subtotal` are not), `promotion_code`, `discount_campaign_name`, `items[]{name, price, quantity, subtotal, note, product{id, product_id, description, photo}}`, `shipping_option`, `banks_available`.
   ...
   - PHOTOS, and the DATES, revised 2026-09-19 after the first real import (invoice 19768, ring R1056). That invoice's item carried **no photo key at all** — the draft stored `source_photo_url: null` with an EMPTY `photo_failures`, so the copy step never ran and the CSR saw a bare placeholder indistinguishable from a failed copy. `product.photo` is therefore NOT reliably present on an invoice line, and the invoice is not a sufficient photo source on its own. `page365-fetch-order` now falls back to the webstore for any product line without a usable photo: (a) `GET /products/<product_id>` with `Accept: application/json`, 6 s; then (b) `GET /products?page=N` from page 1, matching a product whose `name` starts with the line's SKU at a word boundary, capped at 60 pages / 20 s shared across the whole order, with pages cached between lines. `photo.normal` is the webstore's own key and is read through `pickPhotoUrl`. Every outcome is recorded per line in `items[].photo_note` and surfaced in the review screen's placeholder tooltip, naming each step tried and what answered — a missing picture must say why, because neither Claude Code nor Lovable can reach `chajewelsjapan.com` from a sandbox and the owner's screen is the only instrument. A URL is stored ONLY when it parsed out of real JSON; nothing is ever guessed. Separately, the invoice's `created_at` and `expires_on` now travel with the draft as `page365_created_at` / `page365_expires_on`, and the review screen defaults the order date to the invoice's own date — rendered as the **Tokyo** calendar date, since Page365 is a Japanese system and an invoice raised after 15:30 UTC falls on a different Manila day, which would shift every installment due date by one. The field stays editable and carries a "from Page365" badge until it is edited.
   ...
   - CURRENCY, CLOSED 2026-09-19 by owner decision: **Page365 invoices are JPY**, item prices and shipping alike. The second-invoice check this line asked for could not be run from the Claude Code sandbox (the egress proxy denies CONNECT to `chajewelsjapan.com:443`), so the question was answered by the owner rather than by evidence — which is the right way round for a question about what the business means, not what a parser sees. The import is therefore yen throughout, and the ACCOUNT currency is the CSR's choice at confirmation: JPY plans store the yen figures unchanged, PHP plans convert at `system_settings.php_jpy_rate`. The rate and the moment it was read travel with the draft (`payload.fx`) so a peso total can be reproduced later — `src/lib/currency-converter.ts` reads `localStorage` with a hardcoded 0.42 fallback and is per-browser, which is not a basis for a customer's total.
   - PHOTOS, CLOSED 2026-09-19: copied into Hub storage, never hotlinked. `promotions/page365/<no>/<n>.<ext>`, the same public bucket and absolute-URL convention `website_product_media` already uses. The Hub URL goes into `{cash_order,layaway_account}_items.image_url`, a column that has existed on both tables since the Shopify work and had never been written by anything. A photo that cannot be copied is left null — a missing picture is cosmetic and is not worth refusing an otherwise sound import.
   - THE JOIN, CLOSED 2026-09-19: no collision risk (manual series peaks at 19777 layaway / 19671 cash; `web_order_number_seq` is at 900026), but `invoice_number` was UNIQUE only WITHIN each table, and `create-layaway-account` did not pre-check even its own. `public.invoice_numbers` + `trg_zz_invoice_registry_*` now make it structural across both. Measured at backfill: 160 cash + 1538 layaway = 1698 rows, 1698 distinct, **0 cross-table duplicates**.
   - CUSTOMER MATCHING, CLOSED 2026-09-19: Page365 gives name, phone and address and no email; live customer phone data is 891/909 present but only 80 clean E.164, with 10 colliding digit-groups over 20 customers. Automatic matching is unsafe, so the CSR confirms a suggested match or creates the customer — a UI decision, and the reason nothing in this backend resolves a customer by itself.
   - PANCAKE PR #20, ANSWERED 2026-09-19: it is a **Pancake POS** holding area, not Page365, and Pancake is push (webhook → `pancake_events` → list) where Page365 is pull (CSR pastes a link), so the list half has no analogue. What was reused is its architecture — *"Orders NEVER auto-create accounts … creation runs through the existing validated flows"* — which is exactly what this backend does. What was NOT reused is its URL-query-param handoff (`{customer_name, amount, currency, pancake_order_id}`), far too thin for item lines, photos, an address and notes; `page365_drafts` is the carrier instead. Separately: `pancake_events` (148 rows) and the `pancake_order_id` columns are LIVE on production although the branch is unmerged, and no order carries one.
