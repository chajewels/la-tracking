...
142. Dashboard collections/today figures included test accounts. dashboard-summary used the stale not-like 'TEST-%' exclusion (which misses the CJ-2026 test family) on most queries, and had NO test filter at all on the two payment sums feeding collections_this_month and payments_today. Migrated all 15 exclusions to canonical filter('...invoice_number','match','^[0-9]+$') and added the account join+filter to todayPayQ/monthPayQ. Pointed the Finance Collections-tab 'This Month' card at server summary.collections_this_month so it matches the Overview card. Also verified get_collection_analytics already uses the canonical (allocated + actual_remaining) source with numeric test-exclusion — no change needed (no-op). Residual follow-up: Collections-tab Today/Yesterday/Week/Year cells still computed client-side over all payments (test-inclusive). Fixed 2026-05-23. (Bug #142, 2026-05-23)
