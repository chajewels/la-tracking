...
### Bug #177 — `customers.portal_pin_hash` readable by authenticated customers (2026-06-07, SQL only, no code deploy)

Scanner (Deep agentic scan) flagged Critical. The `customers` table SELECT RLS policy (`auth_user_id = auth.uid()`) is row-level only; Postgres RLS cannot restrict individual columns. Session-authenticated customers could execute `SELECT portal_pin_hash FROM customers` against their own row, enabling offline cracking of their own PIN hash. Fix: `REVOKE SELECT ON public.customers FROM authenticated`, then `GRANT SELECT` on all columns except `portal_pin_hash`, `portal_pin_attempts`, `portal_pin_locked_until` to `authenticated`. Edge functions (`service_role`) bypass column-level grants and continue reading all three columns normally. Verified via `information_schema.column_privileges` — SELECT absent from all three PIN columns for `authenticated` role.
