...
- **Fix:** New PL/pgSQL RPC `public.unwaive_penalty_atomic(p_waiver_id, p_user_id, p_user_email)` (`SECURITY DEFINER`, `search_path=public`) performs the entire reverse-of-approve as a single transaction: resets `penalty_waiver_requests.status='pending'`, restores `penalty_fees.status='unpaid'`, recalculates `layaway_schedule` and `layaway_accounts` (including reverting `completed` → `overdue`/`active`), inserts audit log. New edge function `unwaive-waiver/index.ts` wraps the RPC with auth + `checkPermission('manage_waivers')`. UI `handleUnwaive` refactored from 90 LOC of direct supabase writes to ~15 LOC edge function invoke.
- **Files:** `supabase/functions/unwaive-waiver/index.ts` (new), `src/pages/Waivers.tsx` (handleUnwaive refactor), `public.unwaive_penalty_atomic` RPC (new in DB)
- **Note:** Mirrors architectural pattern of `approve_redemption_atomic` (2026-06-08) and `void_redemption_atomic` (2026-06-09). Three historical orphans backfilled via SQL: Roselia #18603 (reset earlier 2026-06-10), Monika #17933, Maria #17110.
- **Related:** Bug #194 — `penalty-engine` cron has the same asymmetric write at L362 (programmatic unwaive without resetting waiver_request). Separate scope, scheduled for next session.
