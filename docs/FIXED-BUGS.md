...
FOUR surfaces were reading the unwritten column, not one — found by enumerating every reader rather than fixing the reported screen:
  1. `src/pages/AccountDetail.tsx` — the Hub's plan card. The reported defect.
  2. `customer-portal` layaway lines — fetched and nested; no portal component renders them yet, so nothing is visibly blank today, but it would have been the moment one did.
  3. `customer-portal` CASH lines — `CashOrdersSection.tsx` DOES render this thumbnail, so the portal was showing an empty square for cash web orders even after the Hub's cash fix. A live customer-facing blank that the 2026-09-14 fix missed because it only touched `CashOrderDetail.tsx`.
  4. `website` order lines and plan lines — handed the storefront `null` for an image it already has.
Fix: one helper per side, so the two tables cannot drift into differently-keyed mechanisms — `src/lib/resolve-item-images.ts` (client) and `supabase/functions/_shared/item-images.ts` (edge). `CashOrderDetail`'s inline block was replaced by the shared client helper rather than left as a second copy. Shopify-sourced lines carry no variant and keep whatever `image_url` the webhook wrote.
NOT done, deliberately: no backfill of `image_url`, and `create_web_layaway_atomic` still does not write it — read-time resolution is the mechanism, and adding a write would create a second source that can disagree.
Verified: the helper's exact query (select by the line's `variant_id`, `sort` ascending, first per variant) resolves the N4020 image for BOTH CJ-W-900012 and CJ-W-900013 at `sort = 0`, with quantity, SKU, unit price and line total unchanged on the row.
Surfaces checked and NOT affected: the Hub statement and the portal statement render no line items at all; the invoice generator's items are entered by staff or pulled from Page365 and carry no catalog image.

### Bug #274 — every storefront email was cut off on the right; a 100%-width table cannot be inset with margin (2026-09-15)
Root cause: `@react-email`'s `Section` renders as `<table width="100%">`, and the shared `block`, `card` and `buttonWrap` styles inset it with `margin: … 24px`. A table with an explicit 100% width cannot be inset by horizontal margin — the margin shifts it right while the width keeps it a full 100% wide, so it ends exactly margin-left past the panel. `container` carries `overflow: hidden`, so those pixels are CLIPPED rather than merely spilling.
Observed in Gmail on CJ-W-900013, on BOTH web-layaway emails, and the second is worse:
  - plan-created: Total, Deposit, Term, account number and account holder truncated mid-value — a customer could read neither the amount owed nor the account number to transfer to.
  - payment-received: EVERY row of the PAYMENT SCHEDULE cut to `₱3,47`. Six rows, not one complete figure among them. The due dates sit in the left cell and survived; the amounts sit in the right cell and did not, so the customer could read WHEN to pay and not WHAT. The clipping also hid that the final instalment is ₱3,473 rather than ₱3,471 — every row read identically.
Raised earlier on order-confirmation and not fixed then.
Reproduced from the live `develop` code with the real CJ-W-900013 figures (total ₱29,754, deposit ₱8,926 received, remaining ₱20,828, 6 × ₱3,471 with ₱3,473 last): `₱3,47` is `₱3,471` with its last glyph clipped. The defect is in the SHARED table component, not any one template's markup, which is why both templates and all five order emails show it.
NOT the same defect as #55. That one was `display:flex` collapsing label and value together, and its two-cell-table fix is intact and untouched; the table it produced was always the right shape, and merely sat 24px too far right.
Measured, not reasoned: 18 of 42 measurements (7 templates × 2 variants × 3 viewports at 640/600/400px) put content past the 560px panel, all at exactly +21..24px. `box-sizing: border-box` does NOT help — a table already resolves its width attribute that way and the measurement is unchanged, which is why the guess had to be measured.
Fix: a `Panel` component whose gutter is PADDING ON A `<td>` — padding on a td can only shrink the content area, never grow the box past its container — with the visible border on an auto-width `<div>` inside it. `buttonWrap` drops its horizontal margin (the button is centred, so it looks identical). `headerBar` was already padding-only and measured safe.
SECOND failure mode, found by the adversarial case and nearly missed: a value with no space in it (a long account holder written as one token, an IBAN) cannot be wrapped by the table, so auto layout widens the cell until it fits and the 560px `max-width` cannot hold it — the PANEL ITSELF grew to 778px, i.e. the whole email wider than the window, at every viewport. `wordBreak: break-word` + `overflowWrap: anywhere` on the row cells is therefore load-bearing, not tidying.
Verified two ways. Whole-template sweep: 0 of 42 measurements past the panel, in all 7 templates, at 640/600/400px, with the realistic account holder wrapping to three lines (four at 400px) and fully readable. Per-money-cell sweep on the two layaway emails with the real CJ-W-900013 figures, EN and JA, at 640/600/400px — counting every right-aligned value cell clipped by the panel:

| | payment-received EN | payment-received JA | plan-created EN | plan-created JA |
|---|---|---|---|---|
| before | **6 of 6** | **12 of 12** | **14 of 14** | **28 of 28** |
| after  | 0 of 6 | 0 of 12 | 0 of 14 | 0 of 28 |

Every money cell in the live code was clipped, at every viewport — 12 measurements before, 0 after. payment-received is the harder case (more rows, longer table) and is clean; `₱3,473` on the final row is now distinguishable from the five `₱3,471` rows above it. The strip-every-style-attribute variant is also clean for realistic values. It is NOT clean for an unbreakable token — stripping every style also strips `word-break` and `max-width`, and HTML attributes alone have no word-breaking mechanism, so that combination is unsatisfiable by construction rather than an outstanding defect. Gmail keeps inline CSS on tables and cells; the strip test is a floor for structural bugs like #55, not a test word-breaking can pass.
...
### Bug #260 — forfeiting a pre-loyalty account did not deduct cumulative spend (2026-08-26)

`revoke_loyalty_points` returned NULL when no active point lot matched the source reference, before writing a transaction or touching the member. Accounts created before the loyalty system have no lot, so forfeiting one changed its status while leaving its spend in cumulative_spend_jpy — and wrote no revoked receipt, so a later reactivation had nothing to restore from. 226 live accounts were exposed. Found when invoice 18304 was manually forfeited and customer CJ-2026-01504's cumulative stayed at ¥1,109,824 despite ₱32,180.05 of payments. Fixed by falling back to the caller-supplied p_spend_jpy for manual_forfeit, auto_forfeit and final_forfeit, which manual-forfeit and auto-forfeit-settlement already compute from total_paid at php_jpy_rate. The revoked receipt is written with account_id so reactivate-account can locate it and restore_loyalty_points can re-add the spend from the transaction row. Scoped to forfeiture only: voids, edits and cancellations concern a single transaction and have no lotless basis.

### Bug #261 — partial payments reset the 90-day auto-forfeit guard (2026-08-26)

auto-forfeit-settlement skipped any account with a non-voided payment inside 90 days, testing only recency with no minimum amount or sufficiency check. The guard sits ahead of RULE 2, so a token partial short-circuited forfeiture entirely. Invoice 18304: ₱93.05 paid 2026-07-04 against a ₱6,000 installment (₱5,406.95 still owed) blocked forfeiture until 2026-10-02, despite the first unpaid installment being exactly 3 months overdue. Fixed per owner direction — the clock now resets only from the date of a payment that closed an installment (allocations >= base + penalty + carried). The guard moved below the paidItems/unpaidItems computation, which it needs. fetchAllocations now carries date_paid per allocation, and a new allocRowsBySchedule map exposes the rows. Blast radius measured before deploy: 0 accounts newly forfeitable. Note the guard no longer gates the final-month penalty cap rule, which runs earlier — that rule is now independent of recent payment activity.

### Bug #262 — Ring Resize fee derived from the target size instead of the delta (2026-08-28)

Commit 61a0ca7 made the sign optional in parseRingResizeSize so a bare ".5" would parse. Because the regex takes the first match, descriptions that state the target size before the delta began deriving the wrong number: "R1153 | Resize to 7.5US | +3" returned 7.5 (¥3,000) instead of +3 (¥1,500). Fixed by preferring a signed token and falling back to unsigned only when no signed token exists anywhere in the description. Existing service_jobs rows are not recalculated.

### Bug #263 — payment tracking appends dropped for all non-newest-cohort invoices (2026-09-11)

Root cause: append-payment-tracking targeted only system_settings.payment_tracking_sheet_id (the newest sheet, per Bug #245). Invoices live on their cohort sheet (05. May, 06. June, …), so September payments on May–July invoices returned {ok:true,"invoice not in sheet"} and were dropped silently — 181 of 196 invoices with September payments were unreachable (e.g. 18970 ₱14,722 on 2026-09-04, May sheet SEP blank). Secondary causes: (a) unawaited fetch in review-payment-submission could be killed at isolate shutdown (19444, 19450 — single-allocation, confirmed, blank); (b) confirmedPaymentIds.length === 1 gate skipped every multi-invoice split; (c) additive amount ignored penalty allocations that get_tracking_for_invoices excludes, and overwrote the D/E formulas with literals.

Fix: append-payment-tracking rewritten as an idempotent per-invoice REWRITE from get_tracking_for_invoices across all sheets registered in system_settings.payment_tracking_sheets ([{id,cohort}], newest first; fill-payment-tracking prepends on each run). Body is now { invoice_number } only. Writes only G..(TOTAL-1) on the invoice row; C/D/E/TOTAL formulas untouched. review-payment-submission awaits the call and fires once per confirmed invoice (single and split). Backfill SQL registered the 2026 May–Aug sheets; May–Aug regenerated once to catch up. Not wired (accepted): void-payment / edit-payment-amount / restore-payment / shopify-webhook cash payments — full regeneration covers them; the new function is idempotent so wiring is a one-line follow-up.

Follow-up (2026-09-11): void-payment, edit-payment-amount, restore-payment, void-cash-payment, restore-cash-payment now call the shared helper `_shared/payment-tracking.ts` → refreshPaymentTracking(invoice) before returning, so the tracking sheet reflects voids/edits/restores immediately instead of waiting for the next payment or regen. review-payment-submission refactored onto the same helper (behaviour unchanged). The "Not wired (accepted)" list shrinks to shopify-webhook cash payments only.

### Bug #264 — storefront magic link lands on a blank page; customers receive the Hub-branded sign-in email (2026-09-11)

Symptom: on the cha-jewels-web develop preview the sign-in email arrives, clicking it lands on a blank black page, and the email itself reads "Your login link … log in to Cha Jewels Hub".

Evidence: auth.flow_state held exactly one PKCE row — created 12:47:21 UTC by the storefront's signInWithOtp, auth_code set at 12:47:31 (GoTrue verified the link and issued the code), never exchanged. The code was issued but never reached a working exchangeCodeForSession. The storefront's callback URLs (production, the Vercel develop alias, PR previews) are not documented anywhere as having been added to Supabase Auth's redirect allow-list; a redirect_to outside that list makes GoTrue fall back to the project Site URL — the Hub — whose anonymous landing is the black splash. The sandbox proxy blocks *.supabase.co so the allow-list could not be probed directly; confirmation is part of the Lovable auth-settings message.

Fix, storefront (cha-jewels-web): /auth/callback hardened — surfaces GoTrue's error / error_code params as /login?error=… instead of reading them as "no code"; accepts token_hash + type via verifyOtp as well as the PKCE code, so a link opened in a different browser (in-app mail clients) no longer depends on the code-verifier cookie; every failure path is a redirect, never a 500; and session cookies are applied to whichever redirect is returned — previously they were bound to one response object and lost on the link-failed path, sending a freshly signed-in customer straight back to /login. The login form now shows a JA/EN message for an expired or failed link.

Fix, Hub (auth-email-hook): the hook payload carries no user metadata but every link carries its redirect target (callback_url, or redirect_to inside the verify URL). A magic link whose target host is a storefront host (chajewelsjapan.com, the cha-jewels-web Vercel aliases and previews) is sent by a second handler with From "Cha Jewels" and the new _shared/email-templates/storefront-magic-link.tsx — Japanese then English, subject 「Cha Jewels サインインリンク / Your Cha Jewels sign-in link」, no mention of the Hub. Staff and portal emails are sent by the original handler, unchanged (same From, same templates, same subjects). The audience is chosen by peeking at the request body; the chosen handler still verifies the signature on the untouched request before sending.

Still required (Lovable Cloud auth settings, not code): the storefront redirect URLs must be on the Supabase Auth allow-list. Proposed entries: https://www.chajewelsjapan.com/**, https://chajewelsjapan.com/**, https://cha-jewels-web.vercel.app/**, https://cha-jewels-web-*-cha-jewels.vercel.app/** (project-scoped — never a bare https://*.vercel.app/**, which would let any Vercel deployment receive this project's sign-in codes).

Follow-up 2026-09-13 — still reproducing after the four allow-list entries were added. Data: auth.flow_state row b239b87c (Cynthia, bcd8c2cf) created 00:05:40 UTC by signInWithOtp, auth_code issued 00:05:52, never exchanged; no auth.sessions row since; last_sign_in_at unchanged since 2026-07-11. auth.audit_log_entries has 0 rows in this project and flow_state has no redirect column, so the exact redirect_to is not recoverable from the DB — it is `${window.location.origin}/auth/callback?next=%2Faccount` by construction (login-form.tsx). Root cause of both symptoms at once: the host was the Vercel TEAM alias `cha-jewels-web-cha-jewels.vercel.app`, which neither the allow-list wildcard `https://cha-jewels-web-*-cha-jewels.vercel.app/**` nor the hook regex `^cha-jewels-web-[a-z0-9-]+-cha-jewels\.vercel\.app$` matches — both demand a segment between the two dashes. GoTrue therefore rewrote redirect_to to the Site URL (`https://id-preview--5be237e2-….lovable.app`, the "Lovable auth page", then blank because that page cannot exchange a PKCE code it has no verifier for), and the hook, seeing a lovable.app target, sent the Hub template. Fix (code): STOREFRONT_HOSTS now `^cha-jewels-web-([a-z0-9-]+-)?cha-jewels\.vercel\.app$` — team alias, git-branch and per-deployment hosts, nothing outside the team. Fix (Lovable Cloud auth settings, Cynthia): add `https://cha-jewels-web-cha-jewels.vercel.app/**` and `https://cha-jewels-web-git-*-cha-jewels.vercel.app/**` alongside the existing four. Without the allow-list entry the hook fix alone changes nothing: GoTrue rewrites the target before the hook ever sees it.


Follow-up 2026-09-13 (second) — with the allow-list fixed, a sign-in from the develop preview (`cha-jewels-web-git-develop-cha-jewels.vercel.app`) at 00:48 UTC completed to /account, but the email was STILL the Hub template. That host matches both the old and the new STOREFRONT_HOSTS regex, so the host is not what failed. The hook logged nothing about its decision, so the 00:48 send cannot be explained from logs. Reading the Lovable email SDK (`@lovable.dev/email-js@0.1.0`, `AuthEmailHookData`): the payload carries `url` (the verify link) and an optional `callback_url`. `linkTarget()` returned `callback_url` whenever it was present and never looked at `url`'s `redirect_to` — so any non-storefront `callback_url` (a relay or the project's own callback) hid the real target. Fix: `linkTargets()` collects every candidate (callback_url, its redirect param, url, url's redirect_to, and a redirect_to nested inside that) and the storefront branding fires if ANY host matches; `signup` joins `magiclink` as a storefront-eligible action type (a first-time customer's signInWithOtp produces a signup confirmation, which to the customer is the sign-in link they asked for); and every request now logs one JSON line `{audience, action_type, hosts[]}` — hosts only, never token, path or address — so the next send is explainable from the logs.
...
**Fix** (migration 20260913050000, cancel-cash-order, delete-cash-order, website, CashOrderDetail): one terminal RPC `terminate_web_order_atomic(order, 'expired'|'cancelled', …)` — lock, points reversal (ledger row), store credit per refund decision, status flip (the once-only guard), stock restore, note, audit. `expire_web_order_atomic` delegates to it. The SQL cron and `expire_transfer_orders()` are dropped; `auto-expire-cash-orders` runs hourly at :40 and is the only expiry path (stock, submissions, audit, email). Staff cancel of a web order requires a reason and, when money was received, a refund decision (`refund_status`: refund issued / refund pending / store credit issued / no refund, + `refund_note`; credit minted only for store credit issued); the customer gets the new `order-cancelled` email and sees the reason and decision on /account/orders (storefront #20). Web orders cannot be deleted (trigger + RPC + UI); deleting a Hub cash order now revokes points inside the same transaction.

### Bug #269 — loyalty ledger did not reconcile; double-award guard was read-then-check (2026-09-13)

**Symptom.** 31 of 546 members had `remaining_points` ≠ ledger net (historical restores and backfills wrote lots without ledger rows; revoke wrote −original instead of −remaining). The only real double-award guard was the lot index, and it fired after the earned row and counters were written (invoice 19031, 2026-05-20, was a real double award). The cash confirm path had no compare-and-swap.

**Fix** (same migration + award-loyalty-points + review-payment-submission): `loyalty_award_claims` (PK = order) claimed before the earned row and released on failure; cash confirm flips the submission atomically before inserting the payment; `loyalty_transactions` immutability trigger; `revoke_loyalty_points` records −remaining; one-time `adjusted` rows align the ledger to live lots for the 30 REAL drifted members (note "Imported redemption history from the loyalty sheet before the Hub ledger (pre-2026-05-15)", `synced_to_sheet_at` preset; owner decision A) — the test member CJ-2026-05088 is deliberately EXCLUDED (decision B) and its points are zeroed by hand with a note after the release, so `loyalty_integrity_report()` shows exactly that one row until then; `loyalty_integrity_report()` for any-time reconciliation (see docs/AUDIT-RPCS.md).

### Bug #267 — first storefront order-confirmation send rejected: Lovable project not published (2026-09-13)

**Symptom.** Order CJ-W-900009 placed from the develop alias at 02:53:30 UTC;
no confirmation email. **Log** (`website`, 02:53:33Z):
`{"storefront_email":"order-confirmation","reference":"CJ-W-900009","to":"chajewelsjapan@gmail.com","outcome":"error","detail":"Email API error: 400 {\"type\":\"missing_unsubscribe\",\"title\":\"This project is migrating to Lovable-managed email sending. Publish the project to complete the migration, then retry; do not set unsubscribe_token manually.\" …}"}`.
So the send IS wired into `/checkout/pay` and the test gate passed (owner
address); the email API refused. `get_project` reports `is_published: false`.
The sign-in email through the same API works because auth sends carry the auth
run's `run_id`.

**Fix.** Operational, not code: publish the Lovable project once (Lovable's own
hosting of the Hub; Firebase production is untouched). Recorded in
...
Symptom: POST /checkout/pay on the storefront (develop preview, N4020, Japan address, transfer) at ~00:56 UTC returned 500 and the storefront showed the generic "We could not complete your order". Not quote expiry (three quotes at 00:54:31 / 00:58:01 / 00:59:12 for customer 4201767c = Test Customer, the customer row linked to Cynthia's auth user bcd8c2cf; each valid 30 minutes, none consumed), not stock (website_product_variants.stock_qty for N4020 was 1 throughout), not the transfer gate (that returns 409, which the storefront words differently). Reproduced 01:01 UTC by calling create_web_order_atomic inside a rolled-back DO block: `23503 insert or update on table "cash_order_items" violates foreign key constraint "cash_order_items_product_id_fkey" — Key (product_id)=(bf1e7271-…) is not present in table "products"`. cash_order_items.product_id references public.products (the Shopify sync table) while a web order line carries a website_products id; variant_id already referenced website_product_variants correctly. Every web checkout since the feature shipped hit this (cash_orders WHERE source_channel='web' = 0 rows, cash_order_items = 0 rows) — the RPC's EXCEPTION block only maps its own sentinel RAISEs, so the FK error propagated to the edge function's catch and became a bare 500.

Fix, Hub: migration 20260913010000_web_order_items_website_product.sql adds cash_order_items.website_product_id uuid REFERENCES website_products(id) ON DELETE SET NULL (+ index, comments) and re-creates create_web_order_atomic with the items INSERT writing website_product_id (product_id stays the Shopify reference, NULL on web lines). website/index.ts: GET /orders/:id maps website_product_id back onto the storefront's product_id field; every response now carries x-request-id and every error body request_id (the storefront passes its own through if it has one); the catch logs request id, method, path and the Postgres code/message/details/hint instead of the bare message.

Fix, storefront (cha-jewels-web): HubError carries requestId; checkout actions map by the Hub's error code (quote_expired/quote_already_used/quote_not_found → expired, out_of_stock/variant_missing → sold_out, transfer_unavailable, else failed) instead of by status alone; an expired quote re-prices the same basket and lands on Review with a fresh quote and a notice; sold out returns to Delivery with the sold-out copy; a server failure keeps the generic copy and shows "Reference: <request id>" so the next one is one log lookup away.

Side effect of the reproduction: web_order_number_seq advanced (one number consumed by the rolled-back run; the earlier failed real attempts consumed theirs too). Harmless — invoice numbers are only unique, never contiguous.

### Bug #265 — loyalty redemption allowed on cancelled/closed layaway (2026-09-12)
...
which needed a printed metal stamp at creation, so every wallet/bag/key
case/belt failed `no_metal`; (2) `page365_metals_from_text` matched a stamp
only as a bare word, so K18WG, K18YG/WG, 750WG, 750PG, 18KWG, K18g and SV925
— how Page365 prints most stamps — were never read (on the 2026-09-26
catalogue 164 of 642 listings failed `no_metal`: 35 non-jewelry, 129
jewelry); (3) a skipped or failed row never wrote `result_note`, and the toast
said only "N draft(s) created".

Fix (owner decisions 2026-09-26, migration `20261005100000_page365_auto_land.sql`):
the Create drafts step is removed — new in-stock codes LAND in the Catalog by
themselves, unpublished (docs/PAGE365-IMPORT.md "AUTO-LAND"); item types are
jewelry | watch | accessory from printed words; stamps with colour codes are
read, SV925 → SILVER925, SV → SILVER; the jewelry-stamp rule moved from
creation to publish, so an incomplete piece lands flagged instead of being
refused; every row not landed keeps its reason. Tests:
`docs/sql/20261005_page365_auto_land_local_tests.sql`,
