...
**Fix** (migration 20260913050000, cancel-cash-order, delete-cash-order, website, CashOrderDetail): one terminal RPC `terminate_web_order_atomic(order, 'expired'|'cancelled', …)` — lock, points reversal (ledger row), store credit per refund decision, status flip (the once-only guard), stock restore, note, audit. `expire_web_order_atomic` delegates to it. The SQL cron and `expire_transfer_orders()` are dropped; `auto-expire-cash-orders` runs hourly at :40 and is the only expiry path (stock, submissions, audit, email). Staff cancel of a web order requires a reason and, when money was received, a refund decision (`refund_status` + `refund_note`); the customer gets the new `order-cancelled` email and sees the reason and decision on /account/orders (storefront #20). Web orders cannot be deleted (trigger + RPC + UI); deleting a Hub cash order now revokes points inside the same transaction.
