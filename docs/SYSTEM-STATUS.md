...
- OPEN FINDING: `send-transactional-email` and `cleanup-loyalty-images` have `config.toml` `verify_jwt=true` but their CI deploy steps pass `--no-verify-jwt` — gate state is deploy-path-dependent and their CURRENT live state is UNKNOWN (the CI redeploy never happened — the workflow was inert). 32 functions have no `config.toml` `verify_jwt` entry at all (implicit). Resolution pending the live matrix reported by Lovable (the maintenance workflow was unimplementable — no obtainable token — and was removed).
