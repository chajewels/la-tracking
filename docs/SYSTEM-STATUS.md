...
- Shared edge-function helpers added (`_shared/cors.ts` superset CORS + `_shared/handler.ts` `requireAuth`/`requirePermission` encoding the Bug #168/#170 locked rules); no function migrated yet, zero behavior change; the `_shared` files bundle into a function only when that function is next deployed via Lovable IDE (the GitHub deploy workflow was proven inert and removed).
...
- RESOLVED 2026-07-05: live verify_jwt state measured for all 79 deployed functions via unauthenticated `{}` POST probe (gateway ON answers `{"code":"UNAUTHORIZED_NO_AUTH_HEADER",...}` — a platform response occurring nowhere in repo code; anything else means the request reached function code = gate OFF). Result: live ON = the 31 config-true functions + upload-proof; live OFF = the remaining 47; zero undeployed. `send-transactional-email` and `cleanup-loyalty-images` are live ON — config prevailed; the former CI `--no-verify-jwt` flags never executed. `upload-proof` config corrected false→true to match its production-verified live state (FIXED-BUGS #3409). The 32 formerly-implicit functions pinned explicitly to their measured `false`. Caveat: whether Lovable's deploy pipeline reads config.toml is unverified — config.toml is the in-repo declaration of measured truth and guards future CLI deploys; re-probe after deploys to re-verify live state.
- Both GitHub Actions Supabase workflows (deploy + maintenance) REMOVED 2026-07-05: proven inert — SUPABASE_ACCESS_TOKEN never existed and cannot be created under Lovable Cloud; an executed deploy step with an empty token exits 1, so the deploy workflow's entire green-run history was skipped steps. It never deployed anything. Lovable IDE is the only edge-function deploy path.
- Migrations baseline shipped: supabase/migrations/ reduced to the single live-introspected 20260705230000_baseline_live_schema.sql (census-verified: 20 enums / 80 tables / 112 functions / 50 triggers / 200 policies / 117 indexes; canaries confirmed incl. allocate_payment_atomic, both redemption atomics, is_test columns, nine-value account_status); the 100 pre-baseline files archived in supabase/migrations-archive/. All 12 live cron jobs captured from cron.job — this also CONFIRMS the sync-backup-sheets cron registration is live at '0 18 * * *' (closes the pending verification on that item).
- Loyalty stale-lots cleanup SHIPPED end-to-end: (C4) loyalty-inactivity-check now retires open lots on expiry + customer-portal filters expired_at (both deployed via Lovable IDE); (C1) backfill reconciled all 90 drifted members atomically — 9 deficit members topped up +13,300 pts via synthetic admin_adjust lots, 81 surplus members cut −177,375 pts FIFO (64 lots stamped expired, 18 consumed, 4 partial) — fleet invariant now 499/499; (C2/C3) approve_redemption_atomic wired to consume_lots_fifo (insufficiency = atomic abort guard) and void_redemption_atomic wired to restore_lots_for_redemption with synthetic top-up for pre-wiring redemptions. Cycle test on the test member caught and led to the fix of Bug #244 (consume from revoked lots); rerun passed all legs. The 62 expired members' phantom portal lots are cleared.
- P4 entry shipped: src/integrations/supabase/types.ts confirmed current (platform auto-regenerates on Lovable schema syncs — 2026-07-05 regen contains allocate_payment_atomic, loyalty_lot_consumption, is_test); all 55 obsolete `(supabase as any)` casts removed across 21 files (typecheck clean), plus the 14 local-alias `= supabase as any` assignments (Commissions/Timesheet/Inquiries/useLoyaltyNotifications) — repo-wide `supabase as any` now 0. strictNullChecks enabled globally (26 pre-measured errors fixed across 12 files; business-rules.ts/calculations.ts required zero changes — money math was already null-clean). Remaining strictness backlog: noImplicitAny/full strict (large, unscheduled).
- P5 (pagination/realtime scoping) re-measured and stays PARKED: 1,166 real layaway accounts / 89 cash orders / 760 customers / 5,636 payments — below the ~1.3k several-fold-growth condition from the original parking decision. useAccounts full-table fetch is fine at this scale; revisit only when real accounts reach ~4-5k. P6 (CustomerPortal decomposition, 3,307 lines) remains unrecommended absent a driving feature.
- Realtime sync extended and verified end-to-end (2026-07-06): supabase_realtime publication +5 tables (loyalty_members, loyalty_transactions, staff_notifications, plus repairs of service_jobs/trade_ins whose SYNC_TABLES subscriptions had been dead since creation — never published); SYNC_TABLES +3; LOYALTY_KEYS + NOTIFICATION_KEYS unioned into REALTIME_INVALIDATE_KEYS. Two follow-up fixes during verification: (1) LOYALTY_KEYS gained 'customer-loyalty-tier' — React Query prefix matching is element-exact, so 'customer-loyalty' never covered the tier badge's key; (2) the global-sync channel is now observable — subscribe() status callback logs SUBSCRIBED/CHANNEL_ERROR and each postgres_changes event logs '[realtime] <table> <eventType>' at console.debug (visible under the Console's Verbose level only). Verified live: websocket frames delivered on UPDATE, and the notification pipeline's chime attempt (AudioContext autoplay warning) fingerprinted the full event→invalidate→refetch→detect chain executing. The bell's 30s/60s polls remain as fallback.
- Tier-benefits drift closed (2026-07-06): staff CustomerLoyaltyTab brought to parity with the portal's DB-first benefits/accent merge (portal side had shipped f72f13d); PENDING item resolved; the tier-benefit edit freeze is lifted.
- portal_token REVOKE no-op (PENDING item) RESOLVED 2026-07-06 as analyzed-accepted: read-only investigation proved the anon read is self-bounded (both anon SELECT policies require portal_token = own request-header token + active-token EXISTS; no unfiltered anon read path; no anon select('*') in frontend). PENDING's original REVOKE+regrant prescription is prohibited by Bug #2302 (breaks PostgREST) and by the policies' USING dependency on portal_token; corrected in PENDING.md with a do-not-attempt note. No code/DB/edge change.
- Portal-token expired-screen bug (OPEN-BUGS.md) RESOLVED 2026-07-06: the entry's header cited a non-existent commit (694d43f) and its body claimed the fix was unbuilt; investigation found part 1 (fetchPortal session→token self-heal) already shipped and part 2 shipped today (4afb8d4) — isExpired narrowed from generic 'expired' to 'token expired' so session-class errors no longer mislabel as 'Portal Link Expired'. Frontend only; the edge function already emitted the distinguishable 'Token expired' signal.
- OPEN-BUGS freshness-gaps note INVESTIGATED/RESOLVED 2026-07-06: the six 'static' cards are already in REALTIME_INVALIDATE_KEYS (live since the realtime work); the Executive 'Live · 30s' badge is accurate (real 30s polling + realtime alert bar); the uncovered Finance keys are heavy aggregate RPCs deliberately kept off realtime to avoid recompute churn on every payment write. No code change warranted.

## 2026-07-05 — allocate_payment_atomic RPC shipped & deployed
- Payment allocation consolidated into the `allocate_payment_atomic` Postgres RPC (single transaction: waterfall + payment insert + payment_allocations + penalty_fees + layaway_schedule + layaway_accounts totals). `review-payment-submission` is the SOLE write-mode caller (`p_preview:false`); `record-payment` (preview_only) and `record-multi-payment` (per-account loop) call it with `p_preview:true`. Shipped commit 136118d; deployed via Lovable IDE (the CI run #274 attributed earlier deployed nothing — inert workflow); frontend via Firebase #1860/#1861. See docs/SCHEMA-FACTS.md for the RPC contract.
- **Verification record:**
...
## Test-account isolation (2026-06-27)

Test accounts are now excluded from all KPIs/alerts via order-level `is_test = false`, not invoice-prefix/regex. Coverage: **24 RPCs + 2 edge functions + 13 frontend sites** swept off both leaky guard patterns (`^[0-9]+$`, `NOT LIKE 'TEST-%'`). Live DB is ahead of repo migrations — column DDL, trigger rewrite, and RPC sweep were applied via SQL Editor and are NOT in supabase/migrations/ (see CLAUDE.md). Re-apply before any `db reset`. Schema in docs/SCHEMA-FACTS.md; root cause in docs/FIXED-BUGS.md Bug #237.

**Known pre-existing quirk (NOT introduced by this work):** `fc_cohort_timeline.collection_rate` can exceed 100% (largest: Mar 2026 6M cohort, 853 accounts, 126.6%) because `actual_collected` includes the downpayment while `expected_collected` (= total_amount − downpayment) excludes it. Surfaces as a noisy "quality degradation" / coverage alert on the Executive Dashboard. Out of scope for the isolation work; separate ticket if the alert is undesired.

## 2026-07-11 — Store Credit system (Phase A) shipped
- Schema: two tables (store_credit_lots, store_credit_transactions) + two enums (store_credit_lot_status, store_credit_txn_type). RLS admin ALL / staff+finance SELECT, no customer-facing policy (portal reads via customer-portal service-role). Lot model, FIFO by soonest expiry, per-currency balances (JPY/PHP never summed or converted), 1-year expiry.
- 4 RPCs (all SECURITY DEFINER, granted to sandbox_exec): issue_store_credit_atomic (mint lot, double-issue guarded), redeem_store_credit_atomic (layaway delegates to allocate_payment_atomic; cash inserts cash_payments directly; ref SC-<uuid>, method store_credit; preview + partial amount), void_store_credit_lot_atomic (unspent remainder only), cancel_cash_order_atomic (cash only — money-received calc, revoke earned points, issue credit, cancel; payments never reversed).
- 4 edge functions: issue-store-credit [issue_store_credit], redeem-store-credit [redeem_store_credit — also calls award-loyalty-points so spent credit earns points], void-store-credit-lot [void_store_credit], cancel-cash-order [cancel_cash_order].
- Staff UI: CustomerStoreCreditTab (balance/lots/history + Issue/Void), IssueStoreCreditDialog, VoidStoreCreditDialog, ApplyStoreCreditCard (AccountDetail + CashOrderDetail, self-hiding), Cancel Order now allowed on COMPLETED cash orders with a preview of credit-to-issue + points-to-revoke. Portal UI: read-only StoreCreditCard on Home (balance, soonest expiry + 30-day warning, recent history; Maison light theme).
...
## 2026-07-13 — Phase C: Hub ↔ Shopify store-credit sync (shipped)
- The Hub's store credit is now mirrored into Shopify's native store-credit account so customers can spend it at Shopify checkout. Model is locked: the HUB MINTS, SHOPIFY MIRRORS — authority one-way, sync bidirectional, Shopify never mints. Full spec: docs/STORE-CREDIT.md Phase C.
- Customer linkage: customers.shopify_customer_id (text, unique partial index where not null). shopify-webhook match chain shopify_customer_id → email → phone → create-and-flag; backfills the link on email/phone matches, never overwrites an existing link.
- consume_store_credit_for_shopify_atomic added — draws Hub lots down FIFO to match a spend that ALREADY happened in Shopify; creates no payment, touches no order, tolerates a shortfall (never fails — the money already moved in Shopify).
- sync-store-credit-to-shopify wired into the FOUR Hub credit functions (issue-store-credit, cancel-cash-order, redeem-store-credit, void-store-credit-lot), each calling it AFTER its RPC succeeds. shopify-webhook is deliberately NOT wired to it — Shopify self-debits at checkout, so pushing a debit would double-debit. Skips silently for non-JPY currency and customers with no shopify_customer_id; a Shopify failure never fails the Hub op (records a store_credit_shopify_sync row, returns 200).
- Shopify-checkout spends: store-credit portion split from real money and recorded as a cash_payments row with payment_method = 'store_credit'; real money recorded one row per gateway; applyShopifyStoreCredit() runs in BOTH orders/create AND orders/paid (Shopify debits at checkout, not payment), idempotent via SHOPIFY-SC-<orderId>; order totals RECOMPUTED from the payment rows.
- New tables: store_credit_shopify_sync (every push attempt — direction, amount, status pending|synced|failed|skipped, shopify_transaction_id, error_detail) and store_credit_reconciliation (one row per customer per run — hub_balance, shopify_balance, delta, status match|drift|shopify_unreadable).
- reconcile-store-credit — REPORT-ONLY drift detector (compares active/unexpired JPY Hub lots vs the Shopify storeCreditAccounts balance; also surfaces pending/failed sync rows). Nightly pg_cron reconcile-store-credit-daily at 18:15 UTC; visible at Settings → Store Credit. It never auto-repairs (drift is a symptom — a human diagnoses).
- Verified end-to-end on live Shopify orders including a MIXED payment (¥50,000 store credit + ¥84,980 bank transfer) and a deliberate drift test that the detector caught and cleared.
- 2026-07-15: revoke_loyalty_points_partial RPC + webhook auto-adjust shipped (commit 5c0e648) and live-verified on SH-1017. Loyalty on partial refunds is now automatic; manual adjustment only needed if the RPC errors (notification says so explicitly).
- 2026-07-15: cancel-race guards + item images shipped (da3f225, b97798f), deployed, verified on SH-1017/1018/1019/1020. cash_order_items.image_url now populated from the products cache on Shopify-synced orders.

## Payment submissions
- 2026-07-15 (7728e2a): Submissions tab — reviewers with confirm_payment can inline-edit submitted_amount on pending rows (submitted/under_review/needs_clarification) to correct entry mistakes before confirming. Blocked for split submissions (would desync payment_submission_allocations) and for confirmed/rejected/cancelled rows. Optimistic UI + audit_logs action 'edit_submitted_amount' (old/new values). Frontend-only; review-payment-submission already reads submitted_amount server-side for both layaway and cash branches, so no edge-function change.
- 2026-07-15 (b9de1f8): Manage Invoice button is now matrix-governed on both layaway (AccountDetail) and cash (CashOrderDetail) — gate changed from hard-coded `isAdmin && can('edit_invoice')` / `(isAdmin || isStaff)` to `can('edit_invoice')`. total_amount stays admin-only via the dialogs' internal non-admin guards (unchanged). Overrides load at app mount, so a granted user must reload to see it. Frontend-only; role_permissions has admin=true, staff/finance/csr=false — grant via per-member override. Note: "Edit Account" (edit_account) has no frontend can() gate, but it IS enforced server-side — the restructure-account edge function requires it (checkPermission "edit_account"). It is distinct from edit_invoice, which gates Manage Invoice. Per-member toggles on edit_account therefore govern Restructure Account, not any button's visibility.
- 2026-07-15 (4a8316f): Manage Invoice (EditAccountDialog) — (1) monthly installments auto-recalculate when Total or Downpayment changes: pool = total − DP − frozen(paid/partial/cancelled row bases), floor-split across editable rows with remainder on the last, prefilled into the row inputs (still individually editable, only differing rows are written on save); mirrors create-layaway-account math. (2) Schedule-edit inputs + Add Installment are disabled with tooltip when the user lacks edit_schedule (the server gate on edit-schedule-item is authoritative and unchanged); prior UX let staff type edits they couldn't save, surfacing only a generic "Edge Function returned a non-2xx status code". (3) That invoke now unwraps the edge function's real error body. Frontend-only, no edge-function change.
...
- pancake-process-events (Phase 2, BRANCH feat/pancake-process-events — NOT on main, NOT deployed) — drains pending pancake_events into cash_orders + cash_order_items; customer cascade email -> pancake_fb_id -> phone -> create+flag; invoice PKE-<system_id> (SUPERSEDED by the holding area — this function is unused, do not deploy); skips empty shells and cancellations; does not re-sync updates.
...
- InvoiceGeneratorSheet pre-fill is now source-routed by invoice prefix (PKE- -> get-pancake-order, else get-page365-order); identical contracts, one query per invoice; Page365 behaviour unchanged (BRANCH feat/pancake-process-events — NOT on main, NOT deployed).
- Pancake holding area added as first tab under Sales (/sales?tab=pancake) — lists unconfirmed pancake_events (items AND total > 0, cancellations excluded), staff confirm as Cash or Layaway which routes to /cash-orders/new or /accounts/new with pancake_order_id; no auto-creation (BRANCH feat/pancake-process-events — NOT on main, NOT deployed).
- 2026-07-19: Holding area completed and proven end-to-end against real production orders. Cash/Layaway toggle replaces the two buttons (Confirm is disabled until a type is chosen). Orders with any 'processed' event are excluded — Pancake keeps emitting updates after confirmation (a real order emitted one 11 minutes later), which would otherwise resurface a confirmed order. PAID badge is DISPLAY ONLY; no payment is ever written from a Pancake payload.
- 2026-07-19: Invoice = PKE-<system_id> zero-padded to 3 (PKE-069), matching the number the Pancake POS displays. The long internal order_id appears NOWHERE in the Pancake UI and is unmatchable by staff. The long id remains the MACHINE identity in cash_orders.pancake_order_id / layaway_accounts.pancake_order_id (partial unique indexes) — THAT is what prevents double confirmation, not the invoice number. get-pancake-order looks up system_id first, long order_id as fallback for earlier test invoices.
- 2026-07-19: pancake_order_id is written service-role inside create-cash-order / create-layaway-account, NOT client-side. finance holds create_cash_order=true but cash_orders UPDATE is staff/admin only, so a client-side tag would fail silently for finance and allow a second confirmation. create-cash-order also sets source_channel='pancake' (overrides the frontend's social_manual tag); layaway_accounts has no source_channel column.
- 2026-07-19: NewAccount/NewCashOrder hydrate from the ledger when ?pancake_order_id is present — invoice, amount, discount, shipping, line items, and the matched customer. INERT for every other entry path (manual, CA bot) via an early return. Events are marked 'processed' after successful creation; the update carries no LIMIT so all versions of an order flip.
- 2026-07-19: pancake-order-webhook rejects events whose shop_id differs from PANCAKE_POS_SHOP_ID (returns 200 so Pancake does not retry). KNOWN LATENT GAP: extractOrderId falls back to system_id when order_link is missing, which would store a short sequential number in the same column as long ids — never observed, not yet hardened.
- 2026-07-19 DEFERRED: Shopify dedupe (handover 7.3). Shopify still auto-creates into cash; a Shopify sale reaching Pancake can surface in the holding area alongside the order shopify-webhook already made. Fails safe on the invoice unique constraint, but is visible to staff.
- 2026-07-19 KNOWN RESIDUAL: the hydration effect in NewAccount/NewCashOrder does NOT check Pancake order status. Navigating directly to /cash-orders/new?pancake_order_id=<canceled or removed order> still hydrates the form. Unreachable from the holding area (which excludes 6 and 7), so this is a hand-typed URL only. Not guarded.
- 2026-07-19 TEST STATUS: only the cash path has been exercised end-to-end (single item, free shipping, zero discount, one matched customer). NEVER TESTED before merge: layaway hydration, the loyalty required-field gate, multi-item orders, orders with discount or shipping, ambiguous product titles, unmatched customers. See PANCAKE-TEST-PLAN.md. Do NOT treat this integration as verified until section A passes.
