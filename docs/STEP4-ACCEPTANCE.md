...
| G10 | **The row this script never had.** Take a layaway whose **deposit is already confirmed** (`total_paid > 0`) and still `active`. Open its Deadline card | **No Change button.** The card stays and says: "The deposit is confirmed, so this deadline no longer applies and cannot be changed." Call `set_account_deadlines` on it directly and it returns **`already_paid`** — the date is not written and no `audit_logs` row appears. Void the deposit and the deadline is live again. This is the case that sent someone to test the deadline control on CJ-W-900013, whose deposit was already in: before the fix it returned `ok:true`, moved the column and logged it (harness finding 1). Cash orders are deliberately NOT gated — a partially-paid pending order still expires, so its deadline is still moveable |
